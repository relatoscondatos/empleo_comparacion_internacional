...
# Pivot the DataFrame to get 'sex' values as columns
pivoted_emp_df = filtered_emp_df.pivot_table(index=['country', 'countryCode', 'time'], columns='sex', values='value').reset_index()

# Calculate the femalePercentage and handle division by zero
pivoted_emp_df['femalePercentage'] = pivoted_emp_df['Female'] / pivoted_emp_df['Total'].replace(0, pd.NA)
...
gdp_2023_df = gdp_df[gdp_df['year'] == 2023][['countryCode', 'region', 'incomeGroup', 'value']].rename(columns={'value': 'gddppc'})
